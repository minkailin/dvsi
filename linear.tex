...
\begin{align*}
  \omega &= \pm \kappa, \\
  \taus & = \frac{\epsilon\omega}{\tepsilon k _x F_r }
         \equiv \tau_{\mathrm{s}0}.
\end{align*} 
Since $\taus\geq 0$, we require $k_xF_r > 0$ if $\omega = \kappa$;
...
$\sigma = - \omega + \dd \sigma$ with $\dd\sigma = \ii\dd s -
\dd\omega$, $\taus = \tau_{\mathrm{s}0} +
\dd\taus$, and assume the $\dd$ quantities are small in
magnitude. Then 
\begin{align*} 
\dd s = \frac{2\tepsilon\kappa^2 }{1 +
  4\tepsilon^2\tau_{\mathrm{s}0}^2\kappa^2}\dd \taus. 
\end{align*}
Thus given $\tepsilon$ and $k_x$, there exist growing modes
when the stopping time is slightly larger than
$\tau_{\mathrm{s}c}$. These correspond to slowly-growing epicycles
