...
%growth rates vanish as $\varphi \to 0$.  
Thus the streaming instability is indeed associated with a phase lag
between gas and dust density evolutions. 

In Fig. \ref{si_compare_fig} we compare the most unstable SI 
mode as a function of $\taus$ at fixed $K_z=30$ and $\epsilon =3$. The
solid line is obtained from a full, two-fluid linear analysis 
\citep[similar to][]{youdin05a, kowalik13}. The one-fluid results are
obtained by solving the dispersion relation
Eq. \ref{streaming_dispersion}, both numerically (green asterisks) and
analytically with additional approximations and considerations
(organge diamonds). The analytic model is described in Appendix 
\ref{si_dust_rich}. 
...
$\taus\OmK\gtrsim 0.1$. It is interesting to see for large $\taus$ the 
two-fluid phase lag drops, along with the growth rate. Notcie also the
phase lag remains finite as $\taus\to0$ because the optimum $K_x$
diverges (this is explained in Appendix \ref{si_dust_rich}). 
...
line), the current one-fluid framework simplified by the terminal
velocity approximation (green asterisks) and an analytic solution to
the one-fluid dispersion relation in the dust-rich limit (organge
