...
Now, writing $\sigma = \ii s - \omega$, the real and imaginary parts of 
Eq. \ref{kz_zero_si}  are
\begin{align}
\omega^2 - s^2 - \frac{s}{\epsilon\tstop}  - \kappa^2 &= 0, \label{epi_overs1}\\
...
\begin{align}
  s \simeq \epsilon \tstop \left(k_xF_r\tstop\right)^2 - \kappa^2,
\end{align}
Growth is possible for
sufficiently large stopping times, and/or large radial pressure
gradients, and/or sufficiently small radial wavelengths. 
%For small growth rates the real frequency $|\omega| =  
%O(\kappa)$, so these are overstable epicycles. 

It is interesting to note that Eq. \ref{epi_overs1}---\ref{epi_overs2}
