...
In the special case of $k_z=0$, the incompressible dispersion
relation, Eq. \ref{streaming_incompressible} reduces to 
\begin{align}      %streaming_incompressible
\ii \sigma^2 - \frac{\sigma}{\tepsilon \taus}  - \left(\ii \kappa^2 +
...
pure oscillations with $\sigma = -\omega$ provided that 
\begin{align*}
  \omega &= \pm \kappa,\\
  \taus & = \frac{\epsilon\kappa}{\tepsilon k _x\left|F_r\right|}
         \equiv \tau_{\mathrm{s}*}.
\end{align*}
If we linearize about this state of marginal stability by writing
$\sigma = - \kappa + \ii \dd s$ and $\taus = \tau_{\mathrm{s}*} +
\dd\taus$ and assume $|\dd s|\ll \kappa$ then 
\begin{align*} 
\dd s = \frac{2\tepsilon }{1 +
  4\tepsilon^2\tau_{\mathrm{s}*}^2}\dd \taus. 
\end{align*}
Thus given $\tepsilon$ and $k_x$, there are overstable modes with
stopping times slightly larger than $\tau_{\mathrm{s}*}$. 
