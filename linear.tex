...
\begin{align} 
  s = \frac{\left|\sigma\right|^2\imag\left(\Delta P
    \Delta\rho^*/\rho\right)}{2\real\left(\sigma\right)\mathcal{I}^2}, \label{si_check}
\end{align}
as implied by Eq. \ref{thermal_instability} and
Eq. \ref{pdv}. Interestingly, we find that for cases with
$\epsilon>1$, Eq. \ref{si_check} can be satisified with $\Delta P \to
\Delta P - \dd P$, i.e. the advection part of the Lagragian
pressure perturbation is dominant. Conversely, for `linB' with $\epsilon
< 1$, one can take $\Delta P \simeq \dd P$ in Eq. \ref{si_check}. This
suggests that for SI in gas-dominated disks the distinction between
Eulerian and Lagragian presure perturbations is unimportant. 

{\bf note: important to use largrangian pressure pert}
...
as    
\begin{align*} 
\varphi = -\sgn(\omega)\arg\left(\Delta P\Delta\rho^*\right), 
\end{align*}
so that $\varphi > 0 $ indicates pressure lagging behind density, which
is true for all the cases. In particular, `linB' indicates that growth
rates vanish as $\varphi \to 0$.  
Thus the streaming instability stems  
from the gas pressure evolution lagging behind that of the total
density (and hence behind the dust density for isothermal gas). 
