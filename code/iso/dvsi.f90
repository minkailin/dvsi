module global
  ...
  real*8 :: smallh_g, rhog0_power, smallq, dgratio, delta, smalld, tstop
  real*8 :: zmax 
  real*8 :: dlnHg_dlnr
  ...
  real*8, allocatable :: Fr_arr(:), Fz_arr(:), F_dot_deleps_arr(:), divF_arr(:)
...
program dvsi
  ...
  real*8, external  :: Fr, Fz, F_dot_deleps, div_F
  real*8, external  :: stopping_time 
  namelist /params/ smallh_g, rhog0_power, smallq, delta, dgratio, smalld, tstop 
...
subroutine fill_bigmatrix(knum)
  ...
  do i=1, nz 
     ...
     L21(i,:) = -tstop_arr(i)*(eps_arr(i)*divF_arr(i) + F_dot_deleps_arr(i))*T(i,:) 
     L21(i,:) =  L21(i,:) &
  ...
  L55(i,:) = T(i,:)
  R55(i,:) = zero
  !deps = 0, if stopping time is non zero
  if(tstop.ge.0d0) then
     i = 1
     L22(i,:) = T(i,:)
     R22(i,:) = 0d0 
     i = nz
...
subroutine eigenvalue_problem
  ...
  real*8, parameter :: min_rate = 1d-4, max_rate = 1d0 
  ...
  do i=1, bignz
     ...
     if((eigen_re.ge.min_rate).and.(eigen_re.le.max_rate)) then 
