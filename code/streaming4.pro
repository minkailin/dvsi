...
pro streaming4, eta=eta, kz=kz, dgratio=dgratio, tstop=tstop, title=title, yrange=yrange
...
xtitle  =tex2idl('$K_{!Xz}=K_{!Xx}$') +'!X'
ytitle = tex2idl('$s/\Omega_{!XK}$') + '!X'

if not keyword_set(yrange) then yrange = [0d0,max(rate)]

set_plot,'ps'
file = strcompress('streaming4.ps',/remove_all)
...
plot, kz_axis, rate[0,*],xmargin=[8.3,1.7],ymargin=[3.2,1.8], ystyle=0, xstyle=1 $
      ...
      linestyle = 0, ytitle =ytitle, xtickinterval=xtickinterval, ytickinterval=ytickinterval,charthick=2, yrange=yrange, /xlog, xtickformat='logticks_exp', $
      psym=2,symsize=1
...
xyouts, 0.01, 0.1, 'spurious dusty epicycle', charsize=1.5
