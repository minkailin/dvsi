...
!!$     L21(i,:) = -tstop_arr(i)*( &
!!$          (1d0-eps_arr(i))*T(i,:) + zaxis(i)*( (1d0-eps_arr(i))*Tp(i,:) - depsdz_arr(i)*T(i,:) ) &
!!$          )
!!$     L22(i,:) = -tstop_arr(i)*( &
!!$          -T(i,:) - zaxis(i)*Tp(i,:)&
!!$          )
...
!!$  !no pert vert heat flux
!!$   do i = 1, nz, nz-1 
!!$     L51(i,:) = -(1d0 - eps_arr(i))*T(i,:)*(-gz(zaxis(i)))
!!$     L52(i,:) = T(i,:)*(-gz(zaxis(i)))
!!$     L53(i,:) = zero 
!!$     L54(i,:) = zero 
!!$     L55(i,:) = zero 
!!$     
...
subroutine eigenvalue_problem
  ...
  real*8, parameter :: min_rate = 1d-6, max_rate = 1d0, eigen_tol = 1d-6
  real*8 :: rwork(8*bignz), eigen_re, eigen_im, eigen_mag, vz_max, error_in, error_out, vhsq(nz), diffusion(nz), tgrad(nz)
  real*8 :: lhs_int, rhs_int1, rhs_int2, sreal, error
  real*8 :: kx
