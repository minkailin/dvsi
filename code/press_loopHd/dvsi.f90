module global
  ...
  real*8, allocatable :: kaxis(:), Hdaxis(:), rates(:)
...
program dvsi
  ...
  write(10,fmt='(5(e22.15,x))'), dble(nz), smallh_g, kmin, Hdmin, smallq 
  close(10)
...
!      kaxis(j) = kmin + dlogk*(j-1d0)
  ...
  !set up Hd axis (output every result for every value)
  allocate(Hdaxis(nHd))
  dlogdgr = 0d0 
  if(nHd .gt. 1) dlogdgr = log10(Hdmax/Hdmin)/(nHd - 1d0)
  do j=1, nHd 
     Hdaxis(j) = 10d0**(log10(Hdmin) + dlogdgr*(j-1d0))
  enddo
  ...
  do j=1, nHd  
     ...
     dgratio = Zmetal/Hd !so that Zmetal is constant  
     else 
     dgratio = Zmetal 
     endif 
  ...
  beta       = sqrt(beta)
     ...
     write(15,fmt='(3(e22.15,x))'), Hd, dgratio, krad
     ...
     write(6 ,fmt='(4(e9.2,x))'), Hd, krad, growth, freq 
