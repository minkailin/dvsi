...
The terminal velocity approximation, $\bm{v}_\mathrm{d} - \bm{v}_\mathrm{g} = \tstop \nabla P /\rhog$, was employed from the outset. 
%It assumes  
%the dust-gas relative drift $\bm{v}_mathrm{d} - \bm{v}_\mathrm{g} = \tstop \nabla P /\rhog$ 
This is applicable to small particles with 
$\tstop\OmK\ll 1$, but the precise critical value is problem-dependent. For the linear streaming instability,
even $\tstop\OmK \sim 0.1$ can be captured by simplified one-fluid equations 
(Fig. \ref{si_compare_fig}). On the other hand, the simplified equations contain spurious modes that
may overwhelm physical instabilities in certain situations (see \S\ref{spurious_epi}).  %Ultimately, one must check against the full equations. 

However, our thermodynamic interpretation of dust-gas drift does \emph{not} in fact depend on the terminal velocity approximation.  
...
%This source term reflects  
%a heat flux proportional to $\bm{v}_\mathrm{d} - \bm{v}_\mathrm{g}$, which must generally be solved via a separate 
%evolutionary equation \citep{laibe14}. Nevertheless, this does not change the interpretation of dusty gas as a single ideal fluid subject to cooling.  
As an example, for strictly isothermal dusty gas we obtain   
\begin{align}
\frac{DP}{Dt} = - P\nabla\cdot\bm{v} + c_s^2\nabla\cdot\left[\fdust\left(1-\fdust\right)\rho\left(\bm{v}_\mathrm{d} - \bm{v}_\mathrm{g}\right)\right], \label{no_term_vel} 
\end{align}
\citep[see also][]{laibe14}. This equation does not use the terminal velocity approximation (which would give Eq. \ref{eff_energy}), but 
 we see that dust-gas relative drift can be regarded as a heat flux within the mixture. Calculating this heat flux generally requires solving the evolutionary equation for 
$\bm{v}_\mathrm{d} - \bm{v}_\mathrm{g}$. Nevertheless, one can still consider the dusty-gas as a single fluid subject to cooling. 
}
