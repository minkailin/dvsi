...
As discussed in \S\ref{spurious_epi}, the first-order one-fluid 
formulation (Eqs. \ref{masseq}---\ref{tempeq}) can produce
spuriously-growing modes with frequency $|\sigma|\gtrsim \Omega$. This
results from neglecting terms of $O(|\Delta\bm{v}|^2)$. The full set
of one-fluid equations is significantly more complex than our
model \cite[e.g.][]{laibe14}. However, it is possible to improve upon
the first-order equations whilst retaining similarilty to
...
\equiv  \frac{1}{\rho}\nabla\cdot \bm{T},
\end{align}
where $\bm{T}$ is an effective stress tensor. %This term is neglected
%when working to $O(\left|\Delta\bm{v}\right|)$, as we have done
%thusfar.  % In 
% general $\Delta\bm{v}$ obeys an evolutionary equation. In that case, the stress 
% tensor $\bm{T}$ is reminenscent of magnetic tension in
...
then
\begin{align}
\bm{T} = - \frac{1}{4\pi G} \bm{g}\otimes\bm{g}. \label{effective_g}
\end{align}

{\bf tensor virial theorems?}

However, Eq. \ref{effective_g} is just the 
gravitational stress tensor in self-gravitating hydrodynamics
%\footnote{Technically there is an isotropic part, which} 
\citep{lynden-bell72}. 
