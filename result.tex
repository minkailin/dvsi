...
We now present numerical solutions to the linearized equations in a 
vertically stratified disk. We consider perfectly coupled dust 
($\tstop=0$) so that the equilibria defined in \S\ref{eqm} are exact
steady states. In this limit any growing modes must be associated with
the imposed temperature gradient (\S\ref{limits}). We thus 
examine the effect of dust-loading on the VSI, previously studied in 
pure gas disks \citep[][\citetalias{lin15} in this section]{lin15}. 
