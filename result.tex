...
($\tstop=0$) so the equilibria
defined in \S\ref{eqm} are exact steady states.    

Apart from \S\ref{varHd}---\ref{vert_mixed}, we consider constant
...
this limit any growing modes must be associated with the imposed temperature gradient
(\S\ref{limits}). We are thus examining the effect of dust-loading on
the VSI previously studied in pure gas disks 
\citep[][\citetalias{lin15} in this section]{lin15}.   
...
$\epsilon_0$. i.e. it increases with the vertical buoyancy. 
The total number of modes do not change 
significantly. This is in contrast with the effect of increasing
cooling times, for which \citetalias{lin15} find fewer unstable modes.
...
To connect with previous studies, here  we also use 
...
that $H_\epsilon=10^3\Hg$. Then vertical shear $\p_z\Omega$ is then
...
%absence of turbulent/diffusive processes, dust particles 
