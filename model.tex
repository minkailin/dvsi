...
The gravitational potential of the mixture 
$\psi$ is included here for completeness, but we will mostly neglect
it ($\psi \equiv 0$) unless explicitly stated otherwise.  
...
In this work, consider equilibria with $\mathcal{C}\equiv 0$, namely   
\begin{inparaenum}[1)] 
\item 
...
\end{inparaenum} 
Vertical settling is absent in both cases. Alternatively, we are 
assuming that $\tstop$  is sufficiently small such 
that settling occurs on much longer timescales than the instabilities of interest. 
...
gas density gradient for isothermal gas. Thus to enable instability
here we need to violate Eq. \ref{dusty_solberg2}. We explore this
below. 


\subsubsection{Vertically well-mixed dust}
Here we consider the special case where the dust-to-gas ratio is
uniform in $z$. Then the LHS of Eq. \ref{dusty_solberg2} becomes
\begin{align}
-\frac{c_s^4}{(1+\tepsilon)^4}\left(\frac{\p\ln{\rhog}}{\p
    z}\right)^2\left(\frac{\p\epsilon}{\p r}\right)^2<0.
\end{align}
Thus the disk is \emph{unstable}, since there is no effective vertical 
buoyancy to stabilize vertical motions. In fact, this situation is
identical to the pure gas, adiabtic simulations of \cite{nelson13} in
which the gas entropy is vertically uniform but has a radial
gradient. There, \citeauthor{nelson13} indeed find instability. 
...
protoplanetary disks we conside the equilibria described in
\S\ref{eqm}.  

We assume the disk is approximately Keplerian so that $\kappa\simeq\OmK$, and that the vertical gas distribution is
...
we may have growing oscillations. %Writing $\sigma =
...
\subsubsection{Physical requirement of dust-gas drag overstabilities}  
