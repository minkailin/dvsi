...
Under what condition is positive work is done? 
Consider a point in the volume where $\Delta\rho = 1$  
...
increasing. This over-compensation corresponds to positive work done. 
Similarly, from $C$ to $D$ the parcel is contracting but pressure 
(restoring) forces are still dropping, which allows the 
...
When applied to dusty gas, the work done is attributed to finite 
dust-gas drag, which allows relative drift between gas and dust. 
A pressure-density lag of $A$ to $B$ is achieved if, 
after a region reaches maximum total density, its 
gas component is still increasing. This in turn implies a sufficiently
large particle flux \emph{out} of that region. 
