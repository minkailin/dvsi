...
qualitative 
description below, adapted from the more rigorous treatments of stellar 
pulsations by 
\cite{cox67} and lecture notes by \cite{samadi15} and
...
\end{align}
which states that `PdV' work (right hand side) is
provided by dust-drag friction. This can be seen explicitly as
follows. 

The average rate of work done by the dusty-gas is
\begin{align}
...
Our aim is to relate the work done by the mixture, $\mathcal{W}$, 
to the growth rate of oscillation amplitudes, $s$. To do this, we
first evaluate $\mathcal{W}$ assuming pure oscillations (perturbations
with time dependence $e^{\ii\omega t}$ and real $\omega$), then show
that in fact the amplitudes grow if the work done is positive. 

Noting that only products in the perturbations contribute to 
$\mathcal{W}$, we find that for real $\omega$: 
\begin{align}
...
mixture. Eq. \ref{work_real} show that a phase difference between
pressure and density leads to work done. 

Now combining Eq. \ref{work_real}, \ref{pdv} and \ref{thermal_instability}
gives  
\begin{align}
s = \frac{2}{\mathcal{I}^2}\mathcal{W}, \label{growth_work}
\end{align}
where $\mathcal{I}^2>0$  is the integral in the denominator of
Eq. \ref{thermal_instability}, and we have set $\sigma= - \omega$ in
Eq. \ref{pdv} for pure oscillations.  Thus growing modes in fact arise if
positive work is done by the mixture during an oscillation. This in
turn requires pressure to lag density, discussed next.  
...
evolution of a fluid element. In Fig. \ref{pdv_cartoon}, we show that
if pressure lags behind density, then positive
work done ($\mathcal{W}>0$) and would lead to growing oscillations
($s>0$).  
...
Consider a fluid element at maximum density during an oscillation cycle
(point $A$), so that $D\rho/ D t = 0$. If dust-gas drag provides an
effective heating to the fluid element at $A$ (i.e. gas influx)  then
it will experience increasing pressure, $DP/D t>0$. It attains
pressure maximum (point $B$) \emph{after} reaching density maximum.   
This implies pressure maximizes while the parcel is expanding: 
the parcel's path in the `$P$-$V$' plane  is clockwise, as shown in
the bottom panel of Fig. \ref{pdv_cartoon}. This corresponds to
positive work done.  

Our thermodynamic interpretation does not rigorously explain
\emph{why} dust drag causes a pressure lag behind density, but merely shows
that this \emph{must} be the case for any growing modes associated the
dust drag. To understand how dust-drag causes this lag requires an
explicit solution to the linearized equations with detailed treatment
of the function $\mathcal{C}$. However, given the complexity of
$\mathcal{C}$ (see Appendix \ref{lin_dust}), we might generally expect
the system to  support a range of stable and unstable modes, with the
