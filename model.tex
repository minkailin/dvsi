...
\tstop\rhog/\left(\rhog+\rhod\right)$ used in some studies
\citep[e.g.][]{youdin05a,jacquet11}.   

We consider tightly-coupled dust particles with 
...
\begin{align}
  \Delta\bm{v}\equiv\bm{v}_\mathrm{d} - \bm{v}_\mathrm{g} = \frac{\nabla
    P}{\rhog}\tstop, 
\end{align}
\citep{youdin05a,jacquet11}. More generally, though, $\Delta\bm{v}$
obeys an evolutionary   
...
fluid description of the mixture, to $O\left(\tstop\right)$, as:  
\begin{align} 
...
capacity at constant pressure. However, this is not a useful quantity
for (locally) isothermal gas. 
...
With an effective entropy defined this way, many of the 
...
The physical reason for this analogy is that with strong drag, the
dust is almost perfectly entrained in the gas flow, but there is some
gain/loss of dust particles between different gas parcels. This
property is analogous to the entropy of an adiabatic pure gas subject
to heating/cooling: entropy is conserved following a gas parcel,
except if there is heat exchange between a fluid parcel and the
surrounding.   
