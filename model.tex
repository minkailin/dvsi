...
vertical shear due to variations in the dust-to-gas ratio is at 
...
the above ratio $\sim 1/|q|$). Otherwise, vertical shear is 
predominantly associated with  $\p_rc_s^2$. 
...
instabilities in stars \citep{cox67}. We thus suggest an interpretation
of dust-drag instabilities in disks in a similar way. We give a qualitative
description below, based on the more rigorous treatments of stellar 
pulsational instabilities by 
...
Consider the Lagragian perturbation $\Delta$ of a variable $f$, 
related to the Eulerian perturbation by $\Delta f = \delta f + 
\bm{\xi}\cdot\nabla f$, where $\bm{\xi}$ is the Lagragian  
displacement ($\dd v_{x,z} = -\ii\sigma\xi_{x,z}$). %Note that $\Delta
...
pressure and density evolution following a parcel. Now, consider a 
...
effective heating ($\mathcal{C}>0$), then the fluid element is
experiencing increasing pressure, $DP/D t>0$. It will attain
...
positive work (the enclosed area by its `$P$-$V$' curve  is 
...
(and hence compression) at a pressure bump which back-reacts on the
gas flow in such a way to strengthen the pressure bump. A phase
difference was also noted in the numerical calculations of
\cite[][although they consider the full two-fluid system]{youdin07b}.   

We suggest that a useful way to understand  
dust-drag instabilities in protoplanetary disks may be to re-cast the  
problem into that of thermodynamics, since such instabilities have 
already been well-studied in the stellar context. 
