...
The two fluids interact via a drag force parameterized by the relative stopping 
time $\tstop$ such that 
\begin{align}  
...
\end{align}
\citep{laibe14,price15}, where $D/Dt \equiv \p_t + \bm{v}\cdot\nabla$ is the Lagragian
...
\begin{align}
  ...
  \nabla\cdot \bm{v}_\mathrm{g}  + \mathcal{H}\label{real_pressure}
...
Although we have eliminated the true energy
...
\mathcal{C}&\equiv c_s^2 \nabla\cdot\left[\tstop\left(1 -
  ...
  P\right].
\end{align}
Comparing with Eq. \ref{real_pressure}, we see that 
...
correspondence applicable to particles less than mm in size at a few
...
gas density gradient for isothermal gas. Thus for 
...
uniform in $z$ the left-hand-side of Eq. \ref{dusty_solberg2} becomes
\begin{align}
