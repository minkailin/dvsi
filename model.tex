...
That is, if the dust-to-gas ratio varies on a radial lengthscale much less
...
%For $\epsilon\ll 1$ the
%dust-to-gas ratio should vary on a lengthscale shorter than
%$\sqrt{\epsilon}H_\epsilon$. We apply this below.    

PPDs typically have $\epsilon$ of $O(10^{-2})$.  Then 
for thin dust layers, $H_\epsilon \simeq \Hd\ll \Hg$, instability
requires $|\p_r\ln{\epsilon}|^{-1}\ll O(0.1\Hg)$, i.e. a very short
radial length-scale in the dust-to-gas ratio. This is unlikely to
occur. On the other hand, if dust is well-mixed, say $\Hd\simeq
0.99\Hg$, then instability only requires
$|\p_r\ln{\epsilon}|^{-1}\lesssim O(10\Hg)$, which is more realistic. 
...
\subsection{Thermodynamics of dusty instabilities}\label{dust_work}
