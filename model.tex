...
\begin{align}
  \widetilde{\bm{v}} = \frac{\nabla
    P}{\rhog}\tstop 
\end{align}
\citep{jacquet11}. This equation reflect the physical result that dust
particles tend to drift towards pressure maxima. 
...
\end{align}
where $(r,\phi, z)$ are cylindrical co-ordinates centered on the star.   
...
This reduced temperature is lowered with dust-loading. 
%   \tepsilon\right)}$. 
...
vertically isothermal dusty disks to behave as if it had a temperture
\emph{increasing} away from $z=0$.     
...
has $\gamma=1$ we can define an effective (dimensionless)    
...
The terminal velocity approximation applies to short
stopping times, $\tstop\OmK\ll 1$; while the locally isothermal
approximation applies to short cooling times, $t_\mathrm{cool}\OmK\ll
...
\citep{lin15,malygin17}. Combining these results, we estimate that our
framework is best suited for mm (or smaller) sized particles at a few
...
$\mathcal{C}\neq0$.  
%for general solutions to 
%Eq. \ref{steady_momr}---\ref{steady_momz}. 

% Dust particles settle to the disk midplane on a timescale 
...
In order to have a well-defined stability problem, we formally
take equilibria with $\mathcal{C}\equiv 0$, namely      
\begin{inparaenum}[1)] 
\item 
...
occur on much longer timescales than the 
instabilities of interest.  
...
\subsection{Dusty vertical buoyancy}\label{vbuoyancy}
...
Eq. \ref{eff_energy} in place of the dust continuity
equation. Self-gravity is neglected here. Our discussion is based on
the corresponding axisymmetric linearized equations. A variable $X$ is
...
The condition for axisymmtric \emph{stability}, $\sigma^2>0$, 
...
gas density gradient for isothermal gas. {\bf ref for drift} Thus for
instability we need to violate Eq. \ref{dusty_solberg2}. We explore
this below. 


\subsubsection{Vertically well-mixed dust}\label{vert_mixed_theory}
In the special case where the dust-to-gas ratio is
uniform in $z$ the LHS of Eq. \ref{dusty_solberg2} becomes
\begin{align}
-\frac{c_s^4}{(1+\tepsilon)^4}\left(\frac{\p\ln{\rhog}}{\p
    z}\right)^2\left(\frac{\p\epsilon}{\p r}\right)^2<0.
\end{align}
Thus the disk is \emph{unstable}, because there is no effective vertical 
buoyancy to stabilize vertical motions ($N_z=0$). This situation is 
identical to the pure gas, adiabtic simulations of \cite{nelson13}
...
\subsubsection{Application to protoplanetary disks} 
%In order to apply the stability criteria
%Eq. \ref{dusty_solberg1}---\ref{dusty_solberg2} to typical
%protoplanetary disks 
We now consider the specific equilibria described in 
\S\ref{eqm}. We assume the disk is approximately Keplerian so that
...
PPDs. The third term is always destabilizing, but is small in smooth,
thin disks with radial gradients $O(1/r)$,  $h_\mathrm{g}\ll 1$,
...
axisymmetric perturbations, a result valid for arbitrarily thin dust
layers. 
...
behind that in density (or displacements), see Fig. \ref{lag_cartoon}
...
are thus not possible if the gas does not respond to dust-drag
(i.e. no back-reaction). The pressure-density lag shown in
Fig. \ref{lag_cartoon} is achieved if, after a parcel reaches maximum
total density, its  gas content is still increasing (see $A$ to $B$ in
Fig. \ref{pdv_cartoon}). This in turn implies a sufficiently large
particle flux \emph{out} of the parcel.    
