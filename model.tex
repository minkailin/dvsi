...
gradient and dust-gas friction. 
The term on the right-hand-side, owing to the imposed
...
discussion is based on the corresponding linearized equations about an
axisymmetric steady state. These lead to the integral relation,  
...
\end{align}
between the (complex) mode frequency $\sigma$ and the spatial
structure of the disturbances which are prefixed by deltas. 
...
requiring $\sigma^2>0$  gives the Solberg-Hoiland criteria 
...
Eq. \ref{dusty_solberg1}---\ref{dusty_solberg2} are equivalent to the
Solberg-Hoiland criteria for adiabatic gas if one writes the 
entropy $s=\ln{\left[c_s^2\left(1 - \epsilon\right)\right]}$. 
...
\widetilde{H}/H_\mathrm{g}\lesssim 1$. Then the
...
temperature gradient \citep{nelson13,barker15,lin15}. 
...
which represent all instabilities due to large-but-finite dust-gas
friction. The formalism we have adopted give these instabilities a 
thermodynamical interpretation \citep[thermal instabilities in
  stars][]{latter06} because the dust-gas friction enters the problem
as an energy source term. This includes the streaming instability in
