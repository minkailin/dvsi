...
\tikzstyle{process} = [rectangle, minimum width=2cm, minimum height=1cm, text centered, draw=black, fill=orange!30]
  ...
  \begin{center}
    \begin{tikzpicture}[node distance=5cm]
      \node[process](ellip){isothermal gas + dust};
      \node[process,right of=ellip, node distance = 6cm](gi){adiabatic gas};
      \path [line](ellip) -- node [yshift=.8em] {\textcolor{blue}{equivalent}} (gi);
      \path [line](gi) -- node [yshift=-.8em] {for small particles} (ellip);
    \end{tikzpicture}
 \end{center}
