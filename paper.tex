...
\item dust-loading generally stabilizes the vertical shear
  instability, and can be understood as a result of dust-induced
  buoyancy forces. 
\end{inparaenum}  
%growth rates, and 
...
equivalence in finding dusty analogs of other hydrodynamic
instabilities, as well as a way to simulate dusty protoplanetary disks
using purely hydrodynamic codes with minimal effort. 
...
interest, allowing us to interpret dusty phenomenon through standard
hydrodynamics.   
...
effective entropy of isothermal dusty gas to its dust content. Finite  
dust-gas drag act as effective heating/cooling: the relative
...
layer is. We put forward a thermodynamical thermodynamical interpretation of
...
This paper is organized as follows. We first explain in  
\S\ref{grow_osc} how a pressure-density phase lag in fluids can lead to
instability. This motives us to find an analogy between dusty gas and
pure hydrodynamics, as it would allow us to understand dust dynamics
within the framework of standard hydrodynamics. 

We begin our formal discussion in in \S\ref{setup} by listing the 
one-fluid equations describing tightly-coupled dusty gas. We show that for a
fixed equation of state, these are in 
fact the usual hydrodynamic equations in disguise. 
We describe equilibrium solutions in \S\ref{eqm} which are used in
explicit calculations. In \S\ref{limits} we discuss
general stability properties of dusty disks. We then move onto
explicit stablity analyses. In  
\S\ref{linear_problem} we list the linearized equations, and solve 
them for the SI in \S\ref{si}, and in 
 \S\ref{results} study dusty VSI. We 
...
This effect is similar to being pushed downwards on a swing on the way
down. 
...
disk temperature is time-independent but varies with radius
\citep{chiang97}. Then the pressure of a fluid parcel varies with 
