...
zombie vortices), how to simulate dusty protoplanetary disks
with minor tweaks to purely hydrodynamic codes, as well as possible generalizations
...
\citep{goldreich73, chiang08,barranco09,lee10}. These instabilities 
...
\citep{johansen06, nelson10,bai10,yang14,zhu14,gibbons15,simon16,baruteau16}. 
...
%AY: or not? %We first present, without
%derivation, the correspondence between dusty gas and     
%ideal hydrodynamics in \S\ref{key_idea}. 
In \S\ref{grow_osc} 
...
%This means that dusty-gas phenomena, such 
%as SI and VSI, have thermodynamical interpretations.  
We begin our formal discussion in \S\ref{setup} by transforming the
basic equations describing tightly-coupled dusty gas into  hydrodynamic equations with a special cooling function. 
We derive equilibrium solutions in \S\ref{eqm}, which we use in our explicit stability calculations. In \S\ref{limits} we analyze the 
general stability properties of dusty disks. We then move onto
explicit stability analyses. In  
\S\ref{linear_problem} we show that our linearized equations 
reproduce the SGI and SI in the tight coupling regime. In \S\ref{results} we
